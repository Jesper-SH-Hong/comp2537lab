...
app.get('/getuserinfo', (req, res) => {
  User.findOne({
    ...
    if (userinfo.role != "admin") {
      ...
      console.log("admin here2")
      res.send(userinfo)
      // res.redirect("/getAdminInfo")
    }
  })
})
...
app.get('/adminGetAllUsers', function (req, res) {
  User.find({}, function (err, data) { //몽구스의 CRUD. find, remove, updateone, updatemany, create
    if (err) {
...
app.get('/deleteThisUser/:id', function (req, res) { 
  User.findOneAndRemove({ //id 쓰니까 하나만 업뎃하자
...
function isDuplicate(req, res, uid, upwd, role) {
...
app.post('/register', (req, res) => {
  ...
  // console.log(role)
  isDuplicate(req, res, uid, upwd, role);
});
...
app.get('/profile/:id', function (req, res) {
  ...
  https.get(url, function (https_res) { //2번쨰 인자. fn callback: executed when I recieved response from this server.
    ...
    https_res.on('end', function () { //end of receiving data. whenever I stop receiving this chunk, I'll parse that merged data and print.
      ...
      attack_ = data.stats.filter((obj_) => {
        return obj_.stat.name == "attack"
      }).map((k) => {
        return k.base_stat
      });
      defense_ = data.stats.filter((obj_) => {
        return obj_.stat.name == "defense"
      }).map((k) => {
        return k.base_stat
      });
      speed_ = data.stats.filter((obj_) => {
